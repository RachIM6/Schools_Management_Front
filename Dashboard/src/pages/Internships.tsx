...
interface InternshipResponsible extends BaseUser {
  department?: string;
  role?: string;
}

export function Internships() {
  const [responsibles, setResponsibles] = useState<InternshipResponsible[]>([
    {
      id: "1",
      firstName: "David",
      lastName: "Johnson",
  ...
  const [currentResponsible, setCurrentResponsible] =
    useState<InternshipResponsible | null>(null);
  const [isViewMode, setIsViewMode] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");

  // Form state
  const [formData, setFormData] = useState<Partial<InternshipResponsible>>({
    firstName: "",
    lastName: "",
    username: "",
    email: "",
  ...
  const handleEditResponsible = (responsible: InternshipResponsible) => {
    setIsModalOpen(true);
    setIsViewMode(false);
  ...
  const handleViewResponsible = (responsible: InternshipResponsible) => {
    setIsModalOpen(true);
    setIsViewMode(true);
    setCurrentResponsible(responsible);
    ...
    if (currentResponsible) {
      ...
      const updatedResponsibles = responsibles.map((responsible) =>
        responsible.id === currentResponsible.id
          ? ({ ...responsible, ...formData } as InternshipResponsible)
          : responsible
      );
      setResponsibles(updatedResponsibles);
    } else {
      // Create new responsible
      const newResponsible: InternshipResponsible = {
        ...(formData as InternshipResponsible),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Internship Responsibles"
        description="Manage staff responsible for coordinating internship programs."
      />
